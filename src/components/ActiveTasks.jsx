                        ...
                        {tasks.map(task => {
                                    ...
                                    <tr key={task.id} className="bg-blue-50/50 border-b border-blue-100">
                                        ...
                                        <td className="px-1 md:px-4 py-2 md:py-3 text-right">
                                            <div className="flex justify-center md:justify-end gap-0.5 md:gap-2">
                                                <button onClick={() => handleUpdate(task.id)} className="flex items-center justify-center bg-green-100 text-green-700 hover:bg-green-200 p-1.5 md:p-2 rounded-md transition-colors w-6 h-6 md:w-8 md:h-8"><Save className="w-3 h-3 md:w-4 md:h-4" /></button>
                            ...
                            return (
                                <tr key={task.id} className={`transition-colors ${getRowColor(daysActive)}`}>
                                    <td className="px-1 md:px-6 py-2 md:py-4 font-semibold break-words max-w-[70px] md:max-w-none md:min-w-[150px] text-[10px] md:text-sm text-center md:text-left leading-tight">{task.name}</td>
                                    <td className="px-1 md:px-6 py-2 md:py-4 text-slate-700 font-medium whitespace-nowrap text-[10px] md:text-sm text-center md:text-left"><span className="md:hidden">{task.arrivalDate ? task.arrivalDate.substring(5) : ''}</span><span className="hidden md:inline">{task.arrivalDate}</span></td>
                                    <td className="px-1 md:px-6 py-2 md:py-4 text-slate-700 font-medium whitespace-nowrap text-[10px] md:text-sm text-center md:text-left"><span className="md:hidden">{task.taskDate ? task.taskDate.substring(5) : ''}</span><span className="hidden md:inline">{task.taskDate}</span></td>
                                    <td className="px-1 md:px-6 py-2 md:py-4 font-bold whitespace-nowrap text-center md:text-left">
                                        <span className="inline-flex items-center justify-center px-1.5 md:px-2.5 py-0.5 md:py-1 rounded-full bg-white/50 text-[9px] md:text-xs shadow-sm">
                                            <span className="md:hidden">{daysActive}d</span><span className="hidden md:inline">{daysActive} {daysActive === 1 ? 'day' : 'days'}</span>
                                        </span>
                                    </td>
                                    <td className="px-1 md:px-4 py-2 md:py-4 text-right whitespace-nowrap">
                                        <div className="flex justify-center md:justify-end gap-0.5 md:gap-2">
                                            <button onClick={() => startEdit(task)} className="bg-white/50 text-blue-700 hover:bg-blue-100 hover:text-blue-800 p-1 md:p-2 rounded md:rounded-md transition-colors shadow-sm active:scale-95" title="Edit"><Edit2 className="w-3 h-3 md:w-4 md:h-4" /></button>
                                            <button onClick={() => handleDelete(task.id)} className="bg-white/50 text-red-600 hover:bg-red-200 hover:text-red-700 p-1 md:p-2 rounded md:rounded-md transition-colors shadow-sm active:scale-95" title="Delete"><Trash2 className="w-3 h-3 md:w-4 md:h-4" /></button>
                                        </div>
                                    </td>
                                </tr>
                            );
